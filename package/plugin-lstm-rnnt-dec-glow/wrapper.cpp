...
{
    printf("inputs %d %d %d\n", EMBEDDING_WIDTH, HIDDEN_WIDTH, NUM_LAYERS);
    ...
    printf("in h %f %f %f\n", in_h_0[0], in_h_0[1], in_h_0[2]);
    printf("in c %f %f %f\n", in_c_0[0], in_c_0[1], in_c_0[2]);
    time_t start = clock();
    for( int i=0 ; i<len ; ++i)
    {
        ...
        printf("PRECOPIED RESULT - %f %f %f\n", ((float*)hiddenHOutAddr)[0], ((float*)hiddenHOutAddr)[1], ((float*)hiddenHOutAddr)[2]);
    ...
    printf("out h %f %f %f\n", out_h_n[0], out_h_n[1], out_h_n[2]);
    printf("out c %f %f %f\n", out_c_n[0], out_c_n[1], out_c_n[2]);

    printf("time in module %d\n", end - start);
